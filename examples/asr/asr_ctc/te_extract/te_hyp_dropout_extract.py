...
class TranscriptionConfig:
    ...
    dataset_manifest: Optional[str] = "/data/jykang/database/train_other_500.json"  # Path to dataset's JSON manifest
    save_path_1 : Optional[str] = "/data/jykang/NeMo/data/te_hyp_dropout/1/train-other-500"
    save_path_2 : Optional[str] = "/data/jykang/NeMo/data/te_hyp_dropout/2/train-other-500"
    ...
    # General configs
    output_filename: Optional[str] = "/data/jykang/NeMo/data/transcriptions/dropout_train_other_500.json"
    batch_size: int = 48
    num_workers: int = 0
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    with autocast():
        with torch.no_grad():
            if partial_audio:
                ...
                transcriptions_1 = asr_model.transcribe(
                ...
                transcriptions_2 = asr_model.transcribe(
                    paths2audio_files=filepaths,
                    batch_size=cfg.batch_size,
                    num_workers=cfg.num_workers,
                    return_hypotheses=return_hypotheses,
    ...
    # Extract teacher softmax and text
    for idx, trans in tqdm(enumerate(transcriptions_1)):
        te_hyp = [trans.text]
        base_file_name = os.path.basename(filepaths[idx])
        file_name = os.path.splitext(base_file_name)[0]
        np.save(os.path.join(cfg.save_path_1, file_name), te_hyp)
    
    for idx, trans in tqdm(enumerate(transcriptions_2)):
        te_hyp = [trans.text]
        base_file_name = os.path.basename(filepaths[idx])
        file_name = os.path.splitext(base_file_name)[0]
    ...
    transcriptions = transcriptions_2
