...
class TranscriptionConfig:
    ...
    dataset_manifest: Optional[str] = "/home/jykang/database/check.json"  # Path to dataset's JSON manifest
    ...
    softmax_path : Optional[str] = "/home/jykang/NeMo/ctc_teacher/softmax/dev-clean"
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    for idx, trans in tqdm(enumerate(transcriptions)):
        ...
        # greedy_sft, greedy = torch.max(softmax, dim=-1)

        decoder_output = trans.y_sequence
        ...
        te_sft= np.load(os.path.join(cfg.softmax_path, file_name))
        te_sft = torch.from_numpy(te_sft)
        y = torch.argmax(te_sft, dim=-1)
