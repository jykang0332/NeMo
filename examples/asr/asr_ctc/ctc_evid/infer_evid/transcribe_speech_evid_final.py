...
class TranscriptionConfig:
    # Required configs
    model_path: Optional[str] = "/home/jykang/NeMo/nemo_experiments/Conformer-CTC-BPE-evid-ver2/2024-07-05_17-35-18/checkpoints/Conformer-CTC-BPE-evid-ver2.nemo"  # Path to a .nemo file
    pretrained_name: Optional[str] = None  # Name of a pretrained model
    audio_dir: Optional[str] = None  # Path to a directory which contains audio files
    dataset_manifest: Optional[str] = "/home/jykang/database/zeroth_korean.json"  # Path to dataset's JSON manifest
    ...
    softmax_path : Optional[str] = "/home/jykang/NeMo/ctc_teacher/softmax/dev-other"
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    for idx, trans in tqdm(enumerate(transcriptions)):
        # # softmax
        # logits = trans.alignments 
        # softmax = torch.exp(logits)
        # greedy = torch.argmax(softmax, dim=-1)

        # evid
        alpha = trans.y_sequence
        S = torch.sum(alpha, dim=-1, keepdim=True)
        # 1. evid prob
        probs = alpha / S
        # 2. greedy decoding with evid prob
        # greedy_evid = torch.argmax(probs, dim=-1)
        probs_evid, greedy_evid = torch.max(probs, dim=-1)
        # 3. uncertainty
        uncertainty = 129 / S
        uncertainty = uncertainty.squeeze(-1)

        # teacher alignment label
        base_file_name = os.path.basename(filepaths[idx])
        file_name = os.path.splitext(base_file_name)[0]
        file_name = file_name + '.npy'
        # te_label= np.load(os.path.join(cfg.softmax_path, file_name)) # zeroth일때 제외
        # te_label = torch.from_numpy(te_label) # zeroth일때 제외
        # y = torch.argmax(te_label, dim=-1) # zeroth일때 제외


        # Save !!!
        save_path = os.path.join('/home/jykang/NeMo/nemo_experiments/zeroth_korean/', str(idx + 1))
        os.makedirs(save_path, exist_ok=True)
        # np.save(os.path.join(save_path, 'y.npy'), y) # zeroth일때 제외
        np.save(os.path.join(save_path, 'greedy_evid.npy'), greedy_evid.cpu().numpy())
        np.save(os.path.join(save_path, 'probs_evid.npy'), probs_evid.cpu().numpy())
        np.save(os.path.join(save_path, 'u.npy'), uncertainty.cpu().numpy())
        np.save(os.path.join(save_path, 'text_evid.npy'), trans.text)
