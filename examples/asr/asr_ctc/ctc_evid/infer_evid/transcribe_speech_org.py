...
class TranscriptionConfig:
    # Required configs
    model_path: Optional[str] = "/home/jykang/NeMo/nemo_experiments/Conformer-CTC-BPE-label/2024-07-09_21-40-51/checkpoints/Conformer-CTC-BPE-label.nemo"
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    # # Restore checkpoint
    # ckpt_path = '/home/jykang/NeMo/nemo_experiments/Conformer-CTC-BPE--val_wer=0.0482-epoch=96.ckpt'
    ...
    for idx, trans in tqdm(enumerate(transcriptions)):
        logits = trans.y_sequence
        softmax = torch.exp(logits)
        probs_sft, greedy_sft = torch.max(softmax, dim=-1)

        save_path = os.path.join('/home/jykang/NeMo/nemo_experiments/zeroth_korean/', str(idx + 1))
        np.save(os.path.join(save_path, 'probs_sft.npy'), probs_sft.cpu().numpy())
        np.save(os.path.join(save_path, 'greedy_sft.npy'), greedy_sft.cpu().numpy())
        np.save(os.path.join(save_path, 'text_sft.npy'), trans.text)
