...
@hydra_runner(config_path="/data/jykang/NeMo/examples/asr/conf/conformer", config_name="conformer_ctc_bpe")
def main(cfg):
    ...
    with torch.no_grad():
        for key, value in asr_model.state_dict().items():
            ...
            # if 'decoder.decoder_layers.0.weight' in key:
            #     weight_selection = uniform_element_selection(te_dec_weight, value.shape)
            #     asr_model.state_dict()[key].copy_(weight_selection)
            # if 'decoder.decoder_layers.0.bias' in key:
            #     weight_selection = uniform_element_selection(te_dec_bias, value.shape)
            #     asr_model.state_dict()[key].copy_(weight_selection)
    # exit()
    logging.info("Teacher's weights are loaded successfully !!!")
    del te_enc
    del te_preprocessor
