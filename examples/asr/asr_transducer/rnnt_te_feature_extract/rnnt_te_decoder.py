...
class TranscriptionConfig:
    # Required configs
    model_path: Optional[str] = "/data/jykang/NeMo/nemo_experiments/stt_en_conformer_transducer_large_ls.nemo"  # Path to a .nemo file
    pretrained_name: Optional[str] = None  # Name of a pretrained model
    audio_dir: Optional[str] = None  # Path to a directory which contains audio files
    dataset_manifest: Optional[str] = "/data/jykang/NeMo/data/test_other.json"  # Path to dataset's JSON manifest
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    for key, value in asr_model.state_dict().items():
        if 'decoder' in key:
            ...
            dec_weights[key] = value
        if 'joint' in key:
            print(key, value.shape)
            joint_weights[key] = value
    torch.save(dec_weights, '/data/jykang/NeMo/data_rnnt/dec/dec_weights.pt')
    torch.save(joint_weights, '/data/jykang/NeMo/data_rnnt/joint/joint_weights.pt')
    exit()
