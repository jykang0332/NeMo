...
from nemo.collections.asr.models.rnnt_bpe_models_Fitnet_TeDecJoint import EncDecRNNTBPEModel
...
@hydra_runner(config_path="./conf", config_name="conformer_transducer_bpe_TeDecJoint")
def main(cfg):
    ...
    te_dec = torch.load('/data/jykang/NeMo/data_rnnt/dec/dec_weights.pt')
    te_joint = torch.load('/data/jykang/NeMo/data_rnnt/joint/joint_weights.pt')     
    with torch.no_grad():
        for key, value in asr_model.state_dict().items():
            if 'decoder' in key:
                asr_model.state_dict()[key].copy_(te_dec[key])
            if 'joint' in key:
                asr_model.state_dict()[key].copy_(te_joint[key])
