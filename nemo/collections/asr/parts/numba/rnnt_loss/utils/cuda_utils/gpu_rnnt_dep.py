...
from nemo.collections.asr.parts.numba.rnnt_loss.utils.cuda_utils import gpu_rnnt_kernel_dep, reduce
...
class MultiblankGPURNNT(GPURNNT):
    ...
    def compute_cost_and_score(
        ...
        # Compute alphas
        gpu_rnnt_kernel_dep.compute_multiblank_alphas_kernel[self.minibatch_, self.maxU_, self.stream_, 0](
            acts,
            denom,
            self.sigma,
            alphas,
        ...
        if training:
            # Compute betas
            gpu_rnnt_kernel_dep.compute_multiblank_betas_kernel[self.minibatch_, self.maxU_, self.stream_, 0](
                acts,
                denom,
                self.sigma,
                betas,
            ...
            grad_blocks_per_grid = self.minibatch_ * self.maxT_ * self.maxU_
            grad_threads_per_block = gpu_rnnt_kernel_dep.GPU_RNNT_THREAD_SIZE
            gpu_rnnt_kernel_dep.compute_multiblank_grad_kernel[
    ...
    def compute_cost_and_score(
        ...
        if r < self.omega:
            ...
            gpu_rnnt_kernel_dep.compute_tdt_alphas_kernel_dep[self.minibatch_, self.maxU_, self.stream_, 0](
                ...
                alphas,
                llForward,
                input_lengths, # -> tensor([341, 350, 396, 305, 380, 354, 235, 343], device='cuda:0')
                label_lengths, # -> tensor([110, 129, 135,  82,  99, 108,  67, 113], device='cuda:0')
                labels,
                self.minibatch_, # -> 8
                self.maxT_, # -> 396
                self.maxU_, # -> 136
