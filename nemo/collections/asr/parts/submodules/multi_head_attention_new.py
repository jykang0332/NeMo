...
class RelPositionMultiHeadAttention(MultiHeadAttention):
    ...
    def __init__(self, n_head, n_feat, dropout_rate, pos_bias_u, pos_bias_v, max_cache_len=0, tome=False, r=0, lth_layer=0):
    ...
    def forward(self, query, key, value, mask, pos_emb, cache=None, encoded_length=None):
        ...
        with avoid_float16_autocast_context():
            ...
            if self.tome:
                if self.r > 0:
                    ...
                    query, _ = merge_wavg(merge, query, size=None)
                    key = query
                    value = query
                    q, k, v = self.forward_qkv(query, key, value)
                    mask = mask[:, self.r:, self.r:]
...
def bipartite_soft_matching(
    ...
    with torch.no_grad():
        ...
        scores = a @ b.transpose(-1, -2) # (batch, a.shape[1], b.shape[1])
        ...
        # Mask according to encoded length
        if encoded_length is not None:
            for i in range(metric.shape[0]):
                #if encoded_length[i] % 2 == 0:
                #    scores[i, encoded_length[i] // 2:, :] = -math.inf
                #else:
                #    scores[i, encoded_length[i] // 2 + 1:, :] = -math.inf
                scores[i, (encoded_length[i] +1) // 2:, :] = -math.inf
                scores[i, :, encoded_length[i] // 2:] = -math.inf

        node_max, node_idx = scores.max(dim=-1) # (batch, a.shape[1])
        edge_idx = node_max.argsort(dim=-1, descending=True)[..., None] # (batch, a.shape[1], 1)

        unm_idx = edge_idx[..., r:, :]  # Unmerged Tokens (batch, a.shape[1] - r, 1)
        src_idx = edge_idx[..., :r, :]  # Merged Tokens (batch, r, 1)

        unm_idx = unm_idx.sort(dim=1)[0]
        src_idx = src_idx.sort(dim=1)[0]

        dst_idx = node_idx[..., None].gather(dim=-2, index=src_idx) # (batch, r, 1)
    ...
    def merge(x: torch.Tensor, mode="mean") -> torch.Tensor:
        src, dst = x[..., ::2, :], x[..., 1::2, :]
        n, t1, c = src.shape # (batch, a.shape[1], channels)
        unm = src.gather(dim=-2, index=unm_idx.expand(n, t1 - r, c)) # (batch, a.shape[1] - r, channels)
        src = src.gather(dim=-2, index=src_idx.expand(n, r, c)) # (batch, r, channels)
        dst = dst.scatter_reduce(-2, dst_idx.expand(n, r, c), src, reduce=mode) # (batch, b.shape[1], channels)
