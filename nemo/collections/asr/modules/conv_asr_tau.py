...
class ConvASRDecoder(NeuralModule, Exportable, adapter_mixins.AdapterModuleMixin):
    ...
    def forward(self, encoder_output):
        ...
        linear_output = self.decoder_layers(encoder_output).transpose(1, 2)
        # return torch.nn.functional.log_softmax(self.decoder_layers(encoder_output).transpose(1, 2), dim=-1)
        return torch.nn.functional.log_softmax(linear_output, dim=-1), torch.nn.functional.log_softmax(linear_output / 5, dim=-1)
