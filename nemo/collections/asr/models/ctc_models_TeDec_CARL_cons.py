...
from nemo.collections.asr.models.asr_model_TeDec_CARL import ASRModel 
...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.decoder = EncDecCTCModel.from_config_dict(self._cfg.decoder)
        self.decoder_te = EncDecCTCModel.from_config_dict(self._cfg.decoder)

        # Set teacher decoder to not require gradients
        self.decoder.requires_grad_(True)
        self.decoder_te.requires_grad_(False)
    ...
    def setup_training_data(self, train_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._train_dl = self._setup_dataloader_from_config_KD_all(config=train_data_config)
    ...
    def setup_validation_data(self, val_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._validation_dl = self._setup_dataloader_from_config_KD_all(config=val_data_config)
    ...
    def setup_test_data(self, test_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._test_dl = self._setup_dataloader_from_config_KD_all(config=test_data_config)
    ...
    def forward(
        ...
        #teacher decoder
        st_log_probs = self.decoder_te(encoder_output=encoded)
    ...
    def training_step(self, batch, batch_nb):
        ...
        signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, te_feature, te_feature_len = batch
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            log_probs, tedec_log_probs, encoded_len, predictions = self.forward(input_signal=signal, input_signal_length=signal_len)
        
        # teacher feature -> student decoder
        if log_probs.shape[1] != te_feature.shape[1]:
           te_feature = te_feature[:, :log_probs.shape[1], :]
        te_feature = te_feature.transpose(1, 2)
        stdec_log_probs = self.decoder(encoder_output=te_feature)
        ...
        # tedec loss
        # student encoder -> teacher decoder
        tedec_ctc_loss_value = self.loss(
            log_probs=tedec_log_probs, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
        )
        tedec_softmax = torch.exp(tedec_log_probs)
        tedec_error = (tedec_softmax - te_softmax) * encoded_mask.unsqueeze(-1)  # (B, T, 129)
        tedec_skd_loss = torch.mean(torch.sum(torch.norm(tedec_error, p=2, dim=-1).pow(2), dim=-1))

        # stdec loss
        # teacher encoder -> student decoder
        stdec_ctc_loss_value = self.loss(
            log_probs=stdec_log_probs, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
        )
        stdec_softmax = torch.exp(stdec_log_probs)
        stdec_error = (stdec_softmax - te_softmax) * encoded_mask.unsqueeze(-1)  # (B, T, 129)
        stdec_skd_loss = torch.mean(torch.sum(torch.norm(stdec_error, p=2, dim=-1).pow(2), dim=-1))

        # loss_value = ctc_loss_value + skd_loss * 0.25
        loss_value = (ctc_loss_value + skd_loss * 0.25) + (tedec_ctc_loss_value + tedec_skd_loss * 0.25) + (stdec_ctc_loss_value + stdec_skd_loss * 0.25)
        ...
        tensorboard_logs.update(
            {
                ...
                'student_decoder_loss': ctc_loss_value + skd_loss * 0.25,
                'ctc_loss': ctc_loss_value,
                'skd_loss': skd_loss,
                'teacher_decoder_loss': tedec_ctc_loss_value + tedec_skd_loss * 0.25,
                'tedec_ctc_loss': tedec_ctc_loss_value,
                'tedec_skd_loss': tedec_skd_loss,
                'te_fea_st_dec_loss': stdec_ctc_loss_value + stdec_skd_loss * 0.25,
                'stdec_ctc_loss': stdec_ctc_loss_value,
                'stdec_skd_loss': stdec_skd_loss,
