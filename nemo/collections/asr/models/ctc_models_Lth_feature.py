...
__all__ = ['EncDecCTCModel_Lth_feature']


class EncDecCTCModel_Lth_feature(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.preprocessor = EncDecCTCModel_Lth_feature.from_config_dict(self._cfg.preprocessor)
        self.encoder_st = EncDecCTCModel_Lth_feature.from_config_dict(self._cfg.encoder)

        # jykang
        # Define teacher
        with torch.no_grad():
            self.encoder_te = EncDecCTCModelBPE.from_pretrained(model_name="stt_en_conformer_ctc_large_ls").encoder
            self.decoder_te = EncDecCTCModelBPE.from_pretrained(model_name="stt_en_conformer_ctc_large_ls").decoder
        self.encoder_te.freeze()
        self.decoder_te.freeze()

        with open_dict(self._cfg):
            if "feat_in" not in self._cfg.decoder or (
                not self._cfg.decoder.feat_in and hasattr(self.encoder_st, '_feat_out')
            ):
                self._cfg.decoder.feat_in = self.encoder_st._feat_out
        ...
        self.decoder_st = EncDecCTCModel_Lth_feature.from_config_dict(self._cfg.decoder)
        ...
        if hasattr(self._cfg, 'spec_augment') and self._cfg.spec_augment is not None:
            self.spec_augmentation = EncDecCTCModel_Lth_feature.from_config_dict(self._cfg.spec_augment)
        else:
        ...
        self.decoding_st = CTCDecoding(self.cfg.decoding, vocabulary=self.decoder_st.vocabulary)

        # Setup metric with decoding strategy
        self._wer = WER(
            decoding=self.decoding_st,
    ...
    def change_vocabulary(self, new_vocabulary: List[str], decoding_cfg: Optional[DictConfig] = None):
        ...
        if self.decoder.vocabulary == new_vocabulary:
            ...
            del self.decoder
            self.decoder = EncDecCTCModel_Lth_feature.from_config_dict(new_decoder_config)
            del self.loss
            self.loss = CTCLoss(
    ...
    def _setup_dataloader_from_config(self, config: Optional[Dict]):
        ...
        audio_to_text_dataset_filename.inject_dataloader_value_from_model_config(self.cfg, config, key='sample_rate')
        audio_to_text_dataset_filename.inject_dataloader_value_from_model_config(self.cfg, config, key='labels')
        dataset = audio_to_text_dataset_filename.get_audio_to_text_char_dataset_from_config(
            config=config,
    ...
    def forward(
        ...
        encoder_output = self.encoder_st(audio_signal=processed_signal, length=processed_signal_length)
    ...
    def training_step(self, batch, batch_nb):
        ...
        signal, signal_len, transcript, transcript_len, filename = batch
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            log_probs, encoded_len, _, st_feature, te_feature = self.forward(input_signal=signal, input_signal_length=signal_len)
        ...
        # jykang
        # Save Last layer feature of teacher model
        # te_feature = (B, d_te, T) = (B, 512, T)
        # st_feature = (B, d_se, T) = (B, 128, T)
        import numpy as np

        te_feature = te_feature.squeeze()
        te_feature = te_feature.cpu().detach().numpy()
        base_file_name = os.path.basename(filename[0])
        base_file_name = os.path.splitext(base_file_name)[0]
        np.save(f'/home/jykang/NeMo/data/Lth_feature/check/{base_file_name}.npy', te_feature)     
