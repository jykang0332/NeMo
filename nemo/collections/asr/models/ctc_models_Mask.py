...
__all__ = ['EncDecCTCModel_Mask']


class EncDecCTCModel_Mask(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.preprocessor = EncDecCTCModel_Mask.from_config_dict(self._cfg.preprocessor)
        self.encoder = EncDecCTCModel_Mask.from_config_dict(self._cfg.encoder)
        ...
        self.decoder = EncDecCTCModel_Mask.from_config_dict(self._cfg.decoder)

        # self.teacher_encoder = EncDecCTCModel.from_pretrained('QuartzNet15x5Base-En').encoder
        # self.teacher_decoder = EncDecCTCModel.from_pretrained('QuartzNet15x5Base-En').decoder
        ...
        if hasattr(self._cfg, 'spec_augment') and self._cfg.spec_augment is not None:
            self.spec_augmentation = EncDecCTCModel_Mask.from_config_dict(self._cfg.spec_augment)
        else:
    ...
    def change_vocabulary(self, new_vocabulary: List[str], decoding_cfg: Optional[DictConfig] = None):
        ...
        if self.decoder.vocabulary == new_vocabulary:
            ...
            del self.decoder
            self.decoder = EncDecCTCModel_Mask.from_config_dict(new_decoder_config)
            del self.loss
            self.loss = CTCLoss(
                num_classes=self.decoder.num_classes_with_blank - 1,
                zero_infinity=True,
    ...
    def training_step(self, batch, batch_nb):
        ...
        # jykang
        # Masked loss
        # Change teacher_softmax to one-hot vector (B, T, 129)
        teacher_softmax = torch.nn.functional.one_hot(teacher_softmax.argmax(dim=2))
        teacher_softmax[:, :, 128] = 0
        teacher_softmax[:, :, 0] = 0
        teacher_softmax = teacher_softmax.float()

        teacher_softmax_lst, log_probs_lst = [], []
        for i in range(encoded_len.shape[0]):
            teacher_softmax_lst.append(teacher_softmax[i, :encoded_len[i], :])
            log_probs_lst.append(log_probs[i, :encoded_len[i], :])

        teacher_softmax_lst = torch.cat(teacher_softmax_lst, dim=0)
        log_probs_lst = torch.cat(log_probs_lst, dim=0)

        # Mask (be careful with the scaling)
        teacher_softmax_lst = teacher_softmax_lst.cuda()
        masked_loss = - torch.sum(teacher_softmax_lst * log_probs_lst) / B
        ...
        # # jykang
        loss_value = 0.05 * masked_loss + 0.5 * ctc_st_loss_value
