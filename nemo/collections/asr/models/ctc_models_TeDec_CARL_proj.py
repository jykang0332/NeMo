...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.st_to_te_dim = torch.nn.Linear(176, 512)
    ...
    def setup_training_data(self, train_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._train_dl = self._setup_dataloader_from_config_KD(config=train_data_config)
    ...
    def setup_validation_data(self, val_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._validation_dl = self._setup_dataloader_from_config_KD(config=val_data_config)
    ...
    def setup_test_data(self, test_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._test_dl = self._setup_dataloader_from_config_KD(config=test_data_config)
    ...
    def forward(
        ...
        """
        print(self.state_dict()['decoder_te.decoder_layers.0.bias'])
        ...
        encoder_output = self.encoder(audio_signal=processed_signal, length=processed_signal_length)
        encoded_st = encoder_output[0]
        encoded_tedec = self.st_to_te_dim(encoder_output[0].transpose(1, 2))
        encoded_tedec = encoded_tedec.transpose(1, 2)
        encoded_len = encoder_output[1]

        # student decoder
        st_log_probs = self.decoder(encoder_output=encoded_st)

        # teacher decoder
        tedec_log_probs = self.decoder_te(encoder_output=encoded_tedec)

        greedy_predictions = st_log_probs.argmax(dim=-1, keepdim=False)
    ...
    def training_step(self, batch, batch_nb):
        ...
        signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len = batch
        ...
        loss_value = ctc_loss_value + skd_loss * 0.25 + tedec_ctc_loss_value + tedec_skd_loss * 0.25
        ...
        return {'loss': loss_value, 'ctc_loss': ctc_loss_value, 'skd_loss': skd_loss, 'tedec_ctc_loss': tedec_ctc_loss_value, 'tedec_skd_loss': tedec_skd_loss, 'log': tensorboard_logs}
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self._wer.reset()
        metrics.update({'val_loss': loss_value, 'val_ctc_loss': ctc_loss_value, 'val_skd_loss': skd_loss, 'val_tedec_ctc_loss': tedec_ctc_loss_value, 'val_tedec_skd_loss': tedec_skd_loss, 'val_wer_num': wer_num, 'val_wer_denom': wer_denom, 'val_wer': wer})
