...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def training_step(self, batch, batch_nb):
        ...
        signal, signal_len, transcript, transcript_len, text = batch
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            log_probs_1, encoded_len_1, predictions_1 = self.forward(input_signal=signal, input_signal_length=signal_len)
            log_probs_2, encoded_len_2, predictions_2 = self.forward(input_signal=signal, input_signal_length=signal_len)
        ...
        # Greedy decoding
        hyp_1, _ = self.decoding.ctc_decoder_predictions_tensor(
                            log_probs_1, decoder_lengths=encoded_len_1, return_hypotheses=False,
                        )
        hyp_2, _ = self.decoding.ctc_decoder_predictions_tensor(
                            log_probs_2, decoder_lengths=encoded_len_2, return_hypotheses=False,
                        )

        # Stage 2: Teacher preference check
        hyp_1_te, hyp_len_1_te = self.make_transcript_batch(hyp_1, self.teacher_model, signal.device)
        hyp_2_te, hyp_len_2_te = self.make_transcript_batch(hyp_2, self.teacher_model, signal.device)

        with torch.no_grad():
            encoded_te, encoded_len_te = self.teacher_model.forward(input_signal=signal, input_signal_length=signal_len)
            decoded_1, target_len_1, states_1 = self.teacher_model.decoder(targets=hyp_1_te, target_length=hyp_len_1_te)
            decoded_2, target_len_2, states_2 = self.teacher_model.decoder(targets=hyp_2_te, target_length=hyp_len_2_te)
            loss_value_1, _, _, _ = self.teacher_model.joint(
                encoder_outputs=encoded_te,
                decoder_outputs=decoded_1,
                encoder_lengths=encoded_len_te,
                transcripts=hyp_1_te,
                transcript_lengths=target_len_1,
                compute_wer=False,
            )
            loss_value_2, _, _, _ = self.teacher_model.joint(
                encoder_outputs=encoded_te,
                decoder_outputs=decoded_2,
                encoder_lengths=encoded_len_te,
                transcripts=hyp_2_te,
                transcript_lengths=target_len_2,
                compute_wer=False,
            )

            # teacher_model forward
            # teacher model preference check -> likelihood 계산 (hypotheses 길이 고려)
            # DPO loss 계산
            exit()
