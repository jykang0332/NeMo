...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        # # Define Teacher model (Preference check teacher)
        # with torch.no_grad():
        #     self.teacher_model = EncDecRNNTBPEModel.from_pretrained(model_name="stt_en_fastconformer_transducer_xlarge")
        # self.teacher_model.eval()
        # self.teacher_model.freeze()

        # # Define Reference model
        # with torch.no_grad():
        #     # # 1. Distilled student model
        #     # self.ref_model = EncDecCTCModelBPE(cfg)
        #     # cfg_ref = copy.deepcopy(cfg)
        #     # OmegaConf.set_struct(cfg_ref, False)
        #     # cfg_ref.init_from_nemo_model = "/data/jykang/NeMo/nemo_experiments/Conformer-CTC-BPE-SKD/2024-05-18_01-54-39/checkpoints/Conformer-CTC-BPE-SKD.nemo"
        #     # self.ref_model.maybe_init_from_pretrained_checkpoint(cfg_ref)
        #     # del cfg_ref

        #     # 2. CTC teacher model
        #     self.ref_model = EncDecCTCModelBPE.from_pretrained(model_name="stt_en_conformer_ctc_large_ls")
    ...
    def setup_training_data(self, train_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._train_dl = self._setup_dataloader_from_config_GT(config=train_data_config)
    ...
    def setup_validation_data(self, val_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._validation_dl = self._setup_dataloader_from_config_GT(config=val_data_config)
    ...
    def training_step(self, batch, batch_nb):
        ...
        signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, text, te_hyp, te_hyp_dropout_1, te_hyp_dropout_2 = batch
        ...
        # with torch.no_grad():
        #     log_probs_l, encoded_len_l, predictions_l = self.ref_model.forward(input_signal=signal, input_signal_length=signal_len)
        # self.ref_model.eval()
        # with torch.no_grad():
        #     log_probs_ref, encoded_len_ref, predictions_ref = self.ref_model.forward(input_signal=signal, input_signal_length=signal_len)

        # # Greedy decoding
        # hyp_l, _ = self.ref_model.decoding.ctc_decoder_predictions_tensor(
        #                     log_probs_l, decoder_lengths=encoded_len_l, return_hypotheses=False,
        #                 )
        # hyp_w, _ = self.ref_model.decoding.ctc_decoder_predictions_tensor(
        ...
        st_chosen_logps = - self.loss(log_probs=log_probs, targets=hyp_w, input_lengths=encoded_len, target_lengths=hyp_w_len)
        st_rejected_logps = - self.loss(log_probs=log_probs, targets=hyp_l, input_lengths=encoded_len, target_lengths=hyp_l_len)

        # ref model log-likelihood
        ref_chosen_logps = - self.loss(log_probs=log_probs_ref, targets=hyp_w, input_lengths=encoded_len_ref, target_lengths=hyp_w_len)
        ref_rejected_logps = - self.loss(log_probs=log_probs_ref, targets=hyp_l, input_lengths=encoded_len_ref, target_lengths=hyp_l_len)
        ...
        ctc_loss_value = self.loss_gt(
            log_probs=log_probs, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
        )

        loss_value = 0.5 * ctc_loss_value + preference_loss
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        # exclude_idx = [i for i in range(len(hyp_l)) if hyp_l[i] == hyp_w[i]]
        # hyp_l, hyp_l_len = self.make_transcript_batch(hyp_l, self.ref_model, signal.device)
        # hyp_w, hyp_w_len = self.make_transcript_batch(hyp_w, self.ref_model, signal.device)
        ...
        self._wer.reset()
        metrics.update({'val_loss': loss_value, 'val_ctc_loss': ctc_loss_value, 'val_prefernce_loss': preference_loss, 'val_wer_num': wer_num, 'val_wer_denom': wer_denom, 'val_wer': wer})
