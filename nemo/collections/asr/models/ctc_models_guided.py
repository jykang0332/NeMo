...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def training_step(self, batch, batch_nb):
        ...
        te_softmax = te_softmax.argmax(dim=-1)
        te_softmax = torch.nn.functional.one_hot(te_softmax)
        te_softmax[:, :, 0] = 0
        te_softmax[:, :, 128] = 0
        te_softmax = te_softmax.to(torch.float32)
        ...
        for i in range(encoded_len.shape[0]):
            te_mask.append(te_softmax[i, :encoded_len[i], :])
        ...
        loss_value = ctc_loss_value + guided_loss
        ...
        return {'loss': loss_value, 'ctc_loss': ctc_loss_value, 'guided_loss': guided_loss, 'log': tensorboard_logs}
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self._wer.reset()
        metrics.update({'val_loss': loss_value, 'val_ctc_loss': ctc_loss_value, 'val_guided_loss': guided_loss, 'val_wer_num': wer_num, 'val_wer_denom': wer_denom, 'val_wer': wer})
