...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        if hasattr(self._cfg, 'spec_augment') and self._cfg.spec_augment is not None:
            ...
            self.spec_augmentation_2 = EncDecCTCModel.from_config_dict(self._cfg.spec_augment_2)
        else:
    ...
    def setup_training_data(self, train_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._train_dl = self._setup_dataloader_from_config_DPO_frame(config=train_data_config)
    ...
    def setup_validation_data(self, val_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._validation_dl = self._setup_dataloader_from_config_DPO_frame(config=val_data_config)
    ...
    def make_transcript_batch(self, text, model, device):
        
        transcript_te = []
        transcript_len_te = []
        if model == None:
            for i in range(len(text)):
                transcript_i = torch.tensor(self.tokenizer.text_to_ids(text[i]))
                transcript_te.append(transcript_i)
                transcript_len_te.append(transcript_i.shape[0])
        else:
            for i in range(len(text)):
                transcript_i = torch.tensor(model.tokenizer.text_to_ids(text[i]))
                transcript_te.append(transcript_i)
                transcript_len_te.append(transcript_i.shape[0])

        transcript_len_te = torch.tensor(transcript_len_te)    
        max_transcript_len = max(transcript_len_te).item()
        transcript_te_batch = []
        for i in range(len(transcript_te)):
            if transcript_te[i].shape[0] < max_transcript_len:
                pad = (0, max_transcript_len - transcript_te[i].shape[0])
                transcript_te[i] = torch.nn.functional.pad(transcript_te[i], pad, value=0)
            transcript_te_batch.append(transcript_te[i])
        transcript_te_batch = torch.stack(transcript_te_batch)

        transcript_te_batch = transcript_te_batch.to(device)
        transcript_len_te = transcript_len_te.to(device)

        return transcript_te_batch, transcript_len_te
    ...
    # @typecheck()
    def forward(
        self, input_signal=None, input_signal_length=None, processed_signal=None, processed_signal_length=None, aug=False,
    ):
        """
        ...
        if aug:
            # Winner augmentation (no aug)
            encoder_output_2 = self.encoder(audio_signal=processed_signal, length=processed_signal_length)
            encoded_2 = encoder_output_2[0]
            log_probs_2 = self.decoder(encoder_output=encoded_2)

            # Loser augmentation
            processed_signal_aug = self.spec_augmentation_2(input_spec=processed_signal, length=processed_signal_length)
            encoder_output_3 = self.encoder(audio_signal=processed_signal_aug, length=processed_signal_length)
            encoded_3 = encoder_output_3[0]
            log_probs_3 = self.decoder(encoder_output=encoded_3)
        ...
        if aug:
            return (
                log_probs, # original
                log_probs_2, # winner aug
                log_probs_3, # loser aug
                encoded_len,
                greedy_predictions,
            )
        else:
    ...
    def forward_2(
        self, input_signal=None, input_signal_length=None, processed_signal=None, processed_signal_length=None, tau=False
    ):
        has_input_signal = input_signal is not None and input_signal_length is not None
        has_processed_signal = processed_signal is not None and processed_signal_length is not None
        if (has_input_signal ^ has_processed_signal) == False:
        ...
        # if self.spec_augmentation_2 is not None and self.training:
        #     processed_signal = self.spec_augmentation_2(input_spec=processed_signal, length=processed_signal_length)
    ...
    def training_step(self, batch, batch_nb):
        ...
        # signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, te_softmax_tau, te_softmax_tau_len, _, _, _ = batch
        signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, te_softmax_aug, te_softmax_aug_len, _, _, _ = batch
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            # log_probs_2, encoded_len_2, predictions_2 = self.forward_2(input_signal=signal, input_signal_length=signal_len) # no augmentation
            log_probs, log_probs_2, log_probs_3, encoded_len, predictions = self.forward(input_signal=signal, input_signal_length=signal_len, aug=True)
        ...
        if log_probs.shape[1] != te_softmax.shape[1]:
           ...
           te_softmax_aug = te_softmax_aug[:, :log_probs.shape[1], :]
        ...
        log_probs_aug_ref = torch.log(te_softmax_aug)

        # student model log-likelihood
        st_chosen = log_probs_2.gather(2, hyp_w.unsqueeze(-1)).squeeze(-1)
        st_rejected = log_probs_3.gather(2, hyp_l.unsqueeze(-1)).squeeze(-1)

        # ref model log-likelihood
        ref_chosen = log_probs_eval_ref.gather(2, hyp_w.unsqueeze(-1)).squeeze(-1)
        ref_rejected = log_probs_aug_ref.gather(2, hyp_l.unsqueeze(-1)).squeeze(-1)

        st_chosen_logps = torch.zeros(len(encoded_len))
        st_rejected_logps = torch.zeros(len(encoded_len))
        ref_chosen_logps = torch.zeros(len(encoded_len))
        ref_rejected_logps = torch.zeros(len(encoded_len))
        for i in range(len(encoded_len)):
            st_chosen_logps[i] = st_chosen[i, :encoded_len[i]].sum()
            st_rejected_logps[i] = st_rejected[i, :encoded_len[i]].sum()
            ref_chosen_logps[i] = ref_chosen[i, :encoded_len[i]].sum()
            ref_rejected_logps[i] = ref_rejected[i, :encoded_len[i]].sum()

        pi_logratios = st_chosen_logps - st_rejected_logps
        ref_logratios = ref_chosen_logps - ref_rejected_logps

        beta = 0.001

        preference_loss = - F.logsigmoid(beta * (pi_logratios - ref_logratios))
        preference_loss = preference_loss.mean().to(log_probs.device)
        
        # CTC loss
        ctc_loss_value = self.loss(
            log_probs=log_probs, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
        )
        ctc_loss_value_2 = self.loss(
            log_probs=log_probs_2, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
        )
        ctc_loss_value_3 = self.loss(
            log_probs=log_probs_3, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
        )
        # SKD loss
        ...
        st_softmax_2 = torch.exp(log_probs_2)
        error_2 = (st_softmax_2 - te_softmax) * encoded_mask.unsqueeze(-1)  # (B, T, 129)
        skd_loss_2 = torch.mean(torch.sum(torch.norm(error_2, p=2, dim=-1).pow(2), dim=-1))

        st_softmax_3 = torch.exp(log_probs_3)
        error_3 = (st_softmax_3 - te_softmax) * encoded_mask.unsqueeze(-1)  # (B, T, 129)
        skd_loss_3 = torch.mean(torch.sum(torch.norm(error_3, p=2, dim=-1).pow(2), dim=-1))

        # loss_value = ctc_loss_value + 100 * preference_loss + 0.25 * skd_loss
        loss_value = ctc_loss_value + ctc_loss_value_2 + ctc_loss_value_3 + 300 * preference_loss + 0.25 * skd_loss + 0.25 * skd_loss_2 + 0.25 * skd_loss_3
        ...
        tensorboard_logs.update(
            {
                ...
                'ctc_loss_2': ctc_loss_value_2,
                'ctc_loss_3': ctc_loss_value_3,
                'skd_loss': skd_loss,
                'skd_loss_2': skd_loss_2,
                'skd_loss_2': skd_loss_3,
