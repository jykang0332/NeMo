...
class EncDecCTCModel_SEQ(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin, ASRBPEMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.preprocessor = EncDecCTCModel_SEQ.from_config_dict(self._cfg.preprocessor)
        ...
        # define student
        self.encoder_st = EncDecCTCModel_SEQ.from_config_dict(self._cfg.encoder)
        ...
        self.decoder_st = EncDecCTCModel_SEQ.from_config_dict(self._cfg.decoder)
    ...
    def change_vocabulary(self, new_vocabulary: List[str], decoding_cfg: Optional[DictConfig] = None):
        ...
        if self.decoder.vocabulary == new_vocabulary:
            ...
            del self.decoder
            self.decoder = EncDecCTCModel_SEQ.from_config_dict(new_decoder_config)
            del self.loss
            self.loss = CTCLoss(
    ...
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        return {
            "outputs_st": NeuralType(('B', 'T', 'D'), LogprobsType()),
            "encoded_lengths_st": NeuralType(tuple('B'), LengthsType()),
    ...
    def forward(
        ...
        # student
        encoder_output_st = self.encoder_st(audio_signal=processed_signal, length=processed_signal_length)
        encoded_st = encoder_output_st[0]
        encoded_len_st = encoder_output_st[1]
        log_probs_st = self.decoder_st(encoder_output=encoded_st)
        greedy_predictions_st = log_probs_st.argmax(dim=-1, keepdim=False)
