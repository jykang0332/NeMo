...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def forward(
        ...
        processed_signal_org = processed_signal
        ...
        if aug:
            # Loser augmentation
            processed_signal_aug = self.spec_augmentation_2(input_spec=processed_signal_org, length=processed_signal_length)
            encoder_output_2 = self.encoder(audio_signal=processed_signal_aug, length=processed_signal_length)
    ...
    def training_step(self, batch, batch_nb):
        ...
        signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, \
            te_softmax_winner, _, te_softmax_loser, _, _, _, _ = batch
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            log_probs, log_probs_2, encoded_len, predictions = self.forward(input_signal=signal, input_signal_length=signal_len, aug=True)
        ...
        # DPO training
        log_probs_winner_ref = torch.log(te_softmax_winner)
        log_probs_loser_ref = torch.log(te_softmax_loser)

        del te_softmax_winner
        del te_softmax_loser

        valid_indices = (te_align != 128) & (te_align != 0)

        # student model log-likelihood
        st_chosen = log_probs.gather(2, hyp_w.unsqueeze(-1)).squeeze(-1)
        st_rejected = log_probs_2.gather(2, hyp_l.unsqueeze(-1)).squeeze(-1)

        # ref model log-likelihood
        ref_chosen = log_probs_winner_ref.gather(2, hyp_w.unsqueeze(-1)).squeeze(-1)
        ref_rejected = log_probs_loser_ref.gather(2, hyp_l.unsqueeze(-1)).squeeze(-1)
        ...
        for i in range(len(encoded_len)):
            ...
            valid_mask = valid_indices[i, :encoded_len[i]]

            st_chosen_logps[i] = st_chosen[i, :encoded_len[i]][valid_mask].sum()
            st_rejected_logps[i] = st_rejected[i, :encoded_len[i]][valid_mask].sum()
            ref_chosen_logps[i] = ref_chosen[i, :encoded_len[i]][valid_mask].sum()
            ref_rejected_logps[i] = ref_rejected[i, :encoded_len[i]][valid_mask].sum()
        ...
        # dpo_error = pi_logratios - ref_logratios
        # st_chosen_logps - st_rejected_logps - (ref_chosen_logps - ref_rejected_logps)
        # st_chosen_logps - ref_chosen_logps - (st_rejected_logps - ref_rejected_logps)
        dpo_error = (st_chosen_logps - ref_chosen_logps) - (st_rejected_logps - ref_rejected_logps).detach()

        # preference_loss = - F.logsigmoid(beta * (pi_logratios - ref_logratios))
        preference_loss = - F.logsigmoid(beta * (dpo_error))
        ...
        # loss_value = ctc_loss_value + ctc_loss_value_2 + 100 * preference_loss + 0.25 * skd_loss + 0.25 * skd_loss_2
