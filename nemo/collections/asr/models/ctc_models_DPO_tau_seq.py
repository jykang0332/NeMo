...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def forward(
        ...
        log_probs, log_probs_tau = self.decoder(encoder_output=encoded)
    ...
    def training_step(self, batch, batch_nb):
        ...
        #     _, _, text, _, st_hyp = batch
        
        signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, \
            te_softmax_tau, te_softmax_tau_len, text, _, _ = batch
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            log_probs, log_probs_tau, encoded_len, predictions = self.forward(input_signal=signal, input_signal_length=signal_len, tau=True)
        ...
        if log_probs.shape[1] != te_softmax.shape[1]:
           ...
           te_softmax_tau = te_softmax_tau[:, :log_probs.shape[1], :]
        ...
        encoded_len_ref = encoded_len

        st_chosen_logps = - self.loss(log_probs=log_probs, targets=hyp_w, input_lengths=encoded_len, target_lengths=hyp_w_len)
        st_rejected_logps = - self.loss(log_probs=log_probs_tau, targets=hyp_l, input_lengths=encoded_len, target_lengths=hyp_l_len)
        ...
        ref_rejected_logps = - self.loss(log_probs=log_probs_tau_ref, targets=hyp_l, input_lengths=encoded_len_ref, target_lengths=hyp_l_len)
