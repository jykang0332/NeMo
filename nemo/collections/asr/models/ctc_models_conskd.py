...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def training_step(self, batch, batch_nb):
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            log_probs2, encoded_len, predictions = self.forward(input_signal=signal, input_signal_length=signal_len)
            log_probs3, encoded_len, predictions = self.forward(input_signal=signal, input_signal_length=signal_len)

        probs1 = log_probs.exp()
        probs2 = log_probs2.exp()
        probs3 = log_probs3.exp()
        probs_mean = (probs1 + probs2 + probs3) / 3
        ...
        # CTC loss for each forward pass
        loss_value1 = self.loss(
            log_probs=log_probs, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
        )
        loss_value2 = self.loss(
            log_probs=log_probs2, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
        )
        loss_value3 = self.loss(
            log_probs=log_probs3, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
        )

        # SKD loss
        kd_loss_value = torch.mean(torch.norm(torch.reshape((te_softmax - probs_mean), (log_probs.size(0), -1)), 2, dim=-1).pow(2)) / 4

        # Consistency loss
        var_loss1 = torch.mean(torch.norm(torch.reshape((probs1 - probs_mean.detach()), (log_probs.size(0), -1)), 2, dim=-1).pow(2)) / 4
        var_loss2 = torch.mean(torch.norm(torch.reshape((probs2 - probs_mean.detach()), (log_probs.size(0), -1)), 2, dim=-1).pow(2)) / 4
        var_loss3 = torch.mean(torch.norm(torch.reshape((probs3 - probs_mean.detach()), (log_probs.size(0), -1)), 2, dim=-1).pow(2)) / 4

        loss_value = (loss_value1 + loss_value2 + loss_value3) / 3 + (kd_loss_value) + var_loss1 + var_loss2 + var_loss3
