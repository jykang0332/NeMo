...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def setup_training_data(self, train_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._train_dl = self._setup_dataloader_from_config_DPO_frame_basic(config=train_data_config)
    ...
    def setup_validation_data(self, val_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._validation_dl = self._setup_dataloader_from_config_DPO_frame_basic(config=val_data_config)
    ...
    def training_step(self, batch, batch_nb):
        ...
        # signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, \
        #     te_softmax_dropout, te_softmax_dropout_len, text, te_hyp, te_hyp_dropout_1 = batch
        
        signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, \
            st_softmax_eval, st_softmax_eval_len, st_softmax_dropout, st_softmax_dropout_len, text, st_hyp, st_hyp_dropout = batch
        ...
        if log_probs.shape[1] != te_softmax.shape[1]:
           ...
           st_softmax_eval = st_softmax_eval[:, :log_probs.shape[1], :]
           st_softmax_dropout = st_softmax_dropout[:, :log_probs.shape[1], :]

        # DPO training
        # Stage 1 & 2: Preference data generation and Preference check
        st_eval_align = st_softmax_eval.argmax(dim=-1)
        st_dropout_align = st_softmax_dropout.argmax(dim=-1)
        # te_align = te_softmax.argmax(dim=-1)

        hyp_l, hyp_w, exclude_idx = [], [], []
        te_log_probs = torch.log(te_softmax)
        for i in range(len(text)):
            wer_1, _, _, _, _ = word_error_rate_detail([text[i]], [st_hyp[i]])
            wer_2, _, _, _, _ = word_error_rate_detail([text[i]], [st_hyp_dropout[i]])
            # Criterion 1: WER
            if wer_1 > wer_2:
                hyp_l.append(st_eval_align[i])
                hyp_w.append(st_dropout_align[i])
            elif wer_1 < wer_2:
                hyp_l.append(st_dropout_align[i])
                hyp_w.append(st_eval_align[i])
            else: # wer_1 == wer_2
                # Criterion 2: Teacher likelihood
                te_logp_1 = te_log_probs[i].gather(1, st_eval_align[i].unsqueeze(-1)).squeeze(-1)
                te_logp_2 = te_log_probs[i].gather(1, st_dropout_align[i].unsqueeze(-1)).squeeze(-1)

                te_likelihood_1 = te_logp_1[:encoded_len[i]].sum()
                te_likelihood_2 = te_logp_2[:encoded_len[i]].sum()

                if te_likelihood_1 > te_likelihood_2:
                    hyp_l.append(st_dropout_align[i])
                    hyp_w.append(st_eval_align[i])
                elif te_likelihood_1 < te_likelihood_2:
                    hyp_l.append(st_eval_align[i])
                    hyp_w.append(st_dropout_align[i])
                else:
                    hyp_l.append(st_dropout_align[i])
        ...
        hyp_l = torch.stack(hyp_l)
        hyp_w = torch.stack(hyp_w)

        # Stage 3: DPO training
        # log_probs_eval_ref = torch.log(te_softmax)
        st_log_probs_eval = torch.log(st_softmax_eval)

        # student model log-likelihood
        st_chosen = log_probs.gather(2, hyp_w.unsqueeze(-1)).squeeze(-1)
        st_rejected = log_probs.gather(2, hyp_l.unsqueeze(-1)).squeeze(-1)

        # ref model log-likelihood
        ref_chosen = st_log_probs_eval.gather(2, hyp_w.unsqueeze(-1)).squeeze(-1)
        ref_rejected = st_log_probs_eval.gather(2, hyp_l.unsqueeze(-1)).squeeze(-1)
        ...
        # exclude the preference loss for the correct hypothesis
        preference_loss[exclude_idx] = 0.0
        preference_loss = preference_loss.sum() / (len(encoded_len) - len(exclude_idx))
        ...
        loss_value = ctc_loss_value + 10 * preference_loss + 0.25 * skd_loss

        # loss_value = preference_loss
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        # signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, te_softmax_dropout, te_softmax_dropout_len, text, te_hyp, te_hyp_dropout_1 = batch
        signal, signal_len, transcript, transcript_len, te_softmax, te_softmax_len, \
            st_softmax_eval, st_softmax_eval_len, st_softmax_dropout, st_softmax_dropout_len, text, st_hyp, st_hyp_dropout = batch
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            log_probs, encoded_len, predictions = self.forward(
        ...
        self._wer.reset()
        # metrics.update({'val_loss': loss_value, 'val_ctc_loss': ctc_loss_value, 'val_skd_loss':/skd_loss, 'val_prefernce_loss': preference_loss, 'val_wer_num': wer_num, 'val_wer_denom': wer_denom, 'val_wer': wer})
