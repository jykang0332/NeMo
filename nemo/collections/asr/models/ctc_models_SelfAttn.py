...
__all__ = ['EncDecCTCModel_SelfAttn']


class EncDecCTCModel_SelfAttn(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.preprocessor = EncDecCTCModel_SelfAttn.from_config_dict(self._cfg.preprocessor)
        self.encoder = EncDecCTCModel_SelfAttn.from_config_dict(self._cfg.encoder)
        ...
        self.decoder = EncDecCTCModel_SelfAttn.from_config_dict(self._cfg.decoder)
        ...
        if hasattr(self._cfg, 'spec_augment') and self._cfg.spec_augment is not None:
            self.spec_augmentation = EncDecCTCModel_SelfAttn.from_config_dict(self._cfg.spec_augment)
        else:
        ...
        # # for dim change
        # self.st_to_te_dim = torch.nn.Linear(16, 64).to(self.device)
    ...
    def change_vocabulary(self, new_vocabulary: List[str], decoding_cfg: Optional[DictConfig] = None):
        ...
        if self.decoder.vocabulary == new_vocabulary:
            ...
            del self.decoder
            self.decoder = EncDecCTCModel_SelfAttn.from_config_dict(new_decoder_config)
            del self.loss
            self.loss = CTCLoss(
                num_classes=self.decoder.num_classes_with_blank - 1,
                zero_infinity=True,
    ...
    def _setup_dataloader_from_config(self, config: Optional[Dict]):
        ...
        audio_to_text_dataset_SelfAttn.inject_dataloader_value_from_model_config(self.cfg, config, key='sample_rate')
        audio_to_text_dataset_SelfAttn.inject_dataloader_value_from_model_config(self.cfg, config, key='labels')
        dataset = audio_to_text_dataset_SelfAttn.get_audio_to_text_char_dataset_from_config_SelfAttn(
            config=config,
    ...
    def training_step(self, batch, batch_nb):
        ...
        signal, signal_len, transcript, transcript_len, filename, te_attn, te_value = batch
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            log_probs, encoded_len, predictions, st_attn, st_value = self.forward(input_signal=signal, input_signal_length=signal_len)
        ...
        # te_attn (B, H, T, T)
        # st_attn (B, H, T, T)
        # te_value (B, H, T, te_d_k), te_d_k = 512 / 8 = 64
        # st_value (B, H, T, st_d_k), st_d_k = 128 / 8 = 16
        # filename (B,)
        # encoded len (B,)



        # shape matching (B, H, T, T)
        if te_attn.shape[-1] != st_attn.shape[-1]:
           te_attn = te_attn[:, :, :st_attn.shape[-1], :st_attn.shape[-1]]
        # # shape matching (B, H, T, .)
        # if te_value.shape[-2] != st_value.shape[-2]:
        #    te_value = te_value[:, :, :st_value.shape[-2], :]

        # self-attention distillation loss
        attn_error = te_attn - st_attn 
        attn_loss = torch.mean(
            torch.sum(torch.norm(attn_error, p=2, dim=-1).pow(2),
                      dim=-1)
            )

        # # value distillation loss
        # st_value = self.st_to_te_dim(st_value)
        # value_error = te_value - st_value
        # value_loss = torch.mean(
        #     torch.sum(torch.norm(value_error, p=2, dim=-1).pow(2),
        #                 dim=-1)
        #     )
        ...
        # loss_value = attn_loss + 0.001 * value_loss
        loss_value = attn_loss
