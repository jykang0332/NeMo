...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def training_step(self, batch, batch_nb):
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            # # 2. For DPO loss, forward with dropout=0.3
            # for module in self.modules():
            #     if isinstance(module, (torch.nn.Dropout, torch.nn.Dropout1d, torch.nn.Dropout2d, torch.nn.Dropout3d)):
            #         module.p = 0.4
            # log_probs_dropout, _, _ = self.forward(input_signal=signal, input_signal_length=signal_len)
        ...
        # log_probs_dropout_ref = torch.log(te_softmax_dropout)
        ...
        # preference_loss = -F.logsigmoid(beta * logits) * (1 - label_smoothing) - F.logsigmoid(-beta * logits) * label_smoothing
