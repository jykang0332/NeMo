...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def training_step(self, batch, batch_nb):
        ...
        # Final loss
        annealing_coef = min(1.0, self.trainer.current_epoch / 50)
        ...
        evid_loss = evid_loss.squeeze(-1) * encoded_mask
        evid_loss = evid_loss.sum(dim=-1)
        evid_loss = torch.mean(evid_loss)
        ...
        tensorboard_logs.update(
            {
                ...
                'evid_loss': evid_loss,
                'mse_loss': loglikelihood.squeeze(-1).sum(dim=-1).mean(),
                'kld_loss': kl_divergence.squeeze(-1).sum(dim=-1).mean(),
