...
class EncDecRNNTModel(ASRModel, ASRModuleMixin, ExportableEncDecModel):
    ...
    def setup_training_data(self, train_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._train_dl = self._setup_dataloader_from_config_fea_rnnt(config=train_data_config)
    ...
    def setup_validation_data(self, val_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        self._validation_dl = self._setup_dataloader_from_config_fea_rnnt(config=val_data_config)
    ...
    def forward(
        ...
        check = torch.load('/data/jykang/NeMo/data_rnnt/joint/joint_weights.pt')
        if (check['joint.pred.weight'].cpu().detach().numpy() == self.state_dict()['joint.pred.weight'].cpu().detach().numpy()).any():
            pass
        else:
            print('!!!!!!!!!')
            exit()
        ...
        encoded = self.addlayer(encoded.transpose(1, 2))
    ...
    def training_step(self, batch, batch_nb):
        ...
        # Fitnet loss
        encoded = encoded.transpose(1,2)
        if encoded.shape[1] != te_feature.shape[1]:
            te_feature = te_feature[:, :encoded.shape[1], :]
            
        encoded_mask = (torch.arange(encoded.shape[1], device=encoded_len.device)[None, :] < encoded_len[:, None]).float()  # (B, T)
        error = (encoded - te_feature) * encoded_mask.unsqueeze(-1)  # (B, T, 512)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if not self.joint.fuse_loss_wer:
            ...
            # Fused joint step
            _, wer, wer_num, wer_denom = self.joint(
                encoder_outputs=encoded,
