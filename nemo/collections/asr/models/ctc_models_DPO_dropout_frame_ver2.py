...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def training_step(self, batch, batch_nb):
        ...
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            ...
            # 2. For DPO loss, forward with dropout=0.0
            # for module in self.modules():
            #     if isinstance(module, (torch.nn.Dropout, torch.nn.Dropout1d, torch.nn.Dropout2d, torch.nn.Dropout3d)):
            #         module.p = 0.0
            # log_probs_dropout_0, _, _ = self.forward(input_signal=signal, input_signal_length=signal_len)

            # # 3. For DPO loss, forward with dropout=0.3
            # for module in self.modules():
            #     if isinstance(module, (torch.nn.Dropout, torch.nn.Dropout1d, torch.nn.Dropout2d, torch.nn.Dropout3d)):
            #         module.p = 0.3
            # log_probs_dropout_3, _, _ = self.forward(input_signal=signal, input_signal_length=signal_len)
        ...
        if log_probs.shape[1] != te_softmax.shape[1]:
           ...
           te_softmax_dropout = te_softmax_dropout[:, :log_probs.shape[1], :]

        # DPO training
        # Stage 1 & 2: Preference data generation and Preference check
        te_hyp_eval = te_softmax.argmax(dim=-1)
        te_hyp_dropout = te_softmax_dropout.argmax(dim=-1)

        # mask = (te_hyp_eval != te_hyp_dropout)

        hyp_l, hyp_w, exclude_idx = [], [], []
        for i in range(len(text)):
            wer_1, _, _, _, _ = word_error_rate_detail([text[i]], [te_hyp[i]])
            wer_2, _, _, _, _ = word_error_rate_detail([text[i]], [te_hyp_dropout_1[i]])
            if wer_1 > wer_2:
                hyp_l.append(te_hyp_eval[i])
                hyp_w.append(te_hyp_dropout[i])
            else:
                # wer이 같은 경우 hyp_w -> te_hyp_eval
                hyp_l.append(te_hyp_dropout[i])
                hyp_w.append(te_hyp_eval[i])

            if (te_hyp_eval[i].cpu().detach().numpy() == te_hyp_dropout[i].cpu().detach().numpy()).all():
        ...
        ref_rejected = log_probs_eval_ref.gather(2, hyp_l.unsqueeze(-1)).squeeze(-1)

        # st_chosen_logps = (st_chosen * mask).sum(dim=1)
        # st_rejected_logps = (st_rejected * mask).sum(dim=1)
        # ref_chosen_logps = (ref_chosen * mask)
        # ref_rejected_logps = (ref_rejected * mask)
        ...
        # ref_chosen_logps = ref_chosen_logps.to(st_chosen_logps.device)
        # ref_rejected_logps = ref_rejected_logps.to(st_chosen_logps.device)
        ...
        loss_value = ctc_loss_value + 0.25 * preference_loss + 0.25 * skd_loss
