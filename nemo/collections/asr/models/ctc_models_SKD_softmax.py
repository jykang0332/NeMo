...
__all__ = ['EncDecCTCModel_SKD_softmax']


class EncDecCTCModel_SKD_softmax(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.preprocessor = EncDecCTCModel_SKD_softmax.from_config_dict(self._cfg.preprocessor)
        self.encoder = EncDecCTCModel_SKD_softmax.from_config_dict(self._cfg.encoder)
        ...
        self.decoder = EncDecCTCModel_SKD_softmax.from_config_dict(self._cfg.decoder)
        ...
        if hasattr(self._cfg, 'spec_augment') and self._cfg.spec_augment is not None:
            self.spec_augmentation = EncDecCTCModel_SKD_softmax.from_config_dict(self._cfg.spec_augment)
        else:
    ...
    def change_vocabulary(self, new_vocabulary: List[str], decoding_cfg: Optional[DictConfig] = None):
        ...
        if self.decoder.vocabulary == new_vocabulary:
            ...
            del self.decoder
            self.decoder = EncDecCTCModel_SKD_softmax.from_config_dict(new_decoder_config)
            del self.loss
            self.loss = CTCLoss(
                num_classes=self.decoder.num_classes_with_blank - 1,
                zero_infinity=True,
    ...
    def training_step(self, batch, batch_nb):
        ...
        signal, signal_len, transcript, transcript_len, te_softmax = batch
        # te_softmax = (B, T, 129)
        ...
        if log_probs.shape[-2] != te_softmax.shape[-2]:
           te_softmax = te_softmax[:,:log_probs.shape[-2],:]

        # log_probs -> st_softmax
        ...
        skd_loss = torch.mean(
            torch.sum(torch.norm(error, p=2, dim=-1).pow(2),
                      dim=-1)
            )
