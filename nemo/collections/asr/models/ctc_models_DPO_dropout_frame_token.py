...
class EncDecCTCModel(ASRModel, ExportableEncDecModel, ASRModuleMixin, InterCTCMixin):
    ...
    def training_step(self, batch, batch_nb):
        ...
        error_dropout = (st_softmax - te_softmax_dropout) * encoded_mask.unsqueeze(-1)  # (B, T, 129)
        skd_loss_dropout = torch.mean(torch.sum(torch.norm(error_dropout, p=2, dim=-1).pow(2), dim=-1))


        loss_value = ctc_loss_value + preference_loss + 0.25 * skd_loss + 0.25 * skd_loss_dropout
        ...
        tensorboard_logs.update(
            {
                ...
                'skd_loss': skd_loss,
                'skd_loss_dropout': skd_loss_dropout,
