...
__all__ = [
    'AudioToCharDataset_SKD',
    'AudioToBPEDataset_SKD',
...
def _speech_collate_fn(batch, pad_id):
    ...
    if len(packed_batch) == 5:
        _, audio_lengths, _, tokens_lengths, feature = packed_batch
    ...
    # Compute max_feature_len
    max_feature_len = 0
    for i in range(len(feature)):
        max_feature_len = max(max_feature_len, feature[i].shape[1])

    audio_signal, tokens = [], []
    feature = []
    for b in batch:
        sig, sig_len, tokens_i, tokens_i_len, feature_i = b
    ...
    return audio_signal, audio_lengths, tokens, tokens_lengths, feature
...
class _AudioTextDataset_SKD(Dataset):
    """
    ...
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        ...
        return {
            ...
            'feature': NeuralType(('D', 'T'), ProbsType()),
        }
    ...
    def __getitem__(self, index):
        ...
        if self.return_sample_id:
            ...
            output = f, fl, torch.tensor(t).long(), torch.tensor(tl).long(), feature
...
class AudioToCharDataset_SKD(_AudioTextDataset_SKD):
    """
...
class AudioToBPEDataset_SKD(_AudioTextDataset_SKD):
    """
    Dataset that loads tensors via a json file containing paths to audio
    files, transcripts, and durations (in seconds). Each new line is a
    different sample. Example below:
