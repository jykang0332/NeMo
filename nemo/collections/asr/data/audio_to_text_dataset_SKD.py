...
from nemo.collections.asr.data import audio_to_text_SKD, audio_to_text_dali
from nemo.collections.asr.parts.preprocessing.perturb import process_augmentations
from nemo.collections.common.data.dataset import ConcatDataset
from nemo.utils import logging


def inject_dataloader_value_from_model_config_SKD(model_cfg: dict, dataloader_cfg: DictConfig, key: str):
    """
...
def get_concat_char_dataset_SKD(
    ...
    for manifest_filepath in manifest_filepaths:
        ...
        dataset = get_char_dataset_SKD(config=conf, augmentor=augmentor)
...
def get_char_dataset_SKD(config: dict, augmentor: Optional['AudioAugmentor'] = None) -> audio_to_text_SKD.AudioToCharDataset_SKD:
    """
    ...
    dataset = audio_to_text_SKD.AudioToCharDataset_SKD(
...
def get_concat_bpe_dataset_SKD(
    config: dict,
    ...
    for manifest_filepath in manifest_filepaths:
        conf = copy.deepcopy(config)
        conf['manifest_filepath'] = manifest_filepath
        dataset = get_bpe_dataset_SKD(config=conf, tokenizer=tokenizer, augmentor=augmentor)
        datasets.append(dataset)
...
def get_bpe_dataset_SKD(
    ...
    """
    dataset = audio_to_text_SKD.AudioToBPEDataset_SKD(
        manifest_filepath=config['manifest_filepath'],
        tokenizer=tokenizer,
        sample_rate=config['sample_rate'],
        int_values=config.get('int_values', False),
...
def get_tarred_dataset(
    ...
    tokenizer: Optional['TokenizerSpec'] = None,
    augmentor: Optional['AudioAugmentor'] = None,
) -> Union[audio_to_text_SKD.TarredAudioToBPEDataset, audio_to_text_SKD.TarredAudioToCharDataset]:
    """
    ...
    for dataset_idx, (tarred_audio_filepath, manifest_filepath) in enumerate(
        ...
        if tokenizer is None:
            dataset = audio_to_text_SKD.TarredAudioToCharDataset(
            ...
            dataset = audio_to_text_SKD.TarredAudioToBPEDataset(
                audio_tar_filepaths=tarred_audio_filepath,
                manifest_filepath=manifest_filepath,
                tokenizer=tokenizer,
                sample_rate=config['sample_rate'],
...
def get_audio_to_text_char_dataset_from_config_SKD(
...
def get_audio_to_text_bpe_dataset_from_config_SKD(
    config,
    local_rank: int,
    global_rank: int,
    world_size: int,
    ...
    if config.get('is_tarred', False):
        ...
        if 'manifest_filepath' in config and config['manifest_filepath'] is None:
            ...
            return None
        if is_concat:
            dataset = get_concat_bpe_dataset_SKD(
                config=config,
                global_rank=global_rank,
                world_size=world_size,
                tokenizer=tokenizer,
            ...
            dataset = get_bpe_dataset_SKD(config=config, tokenizer=tokenizer, augmentor=augmentor)
    return dataset
...
def get_chain_dataset(datasets, ds_config):
    if len(datasets) > 1:
        if ds_config.get('bucketing_batch_size', None) is not None:
            ...
            for idx, dataset in enumerate(datasets):
                datasets[idx] = audio_to_text_SKD.BucketingDataset(
    ...
    if bucketing_strategy == 'fixed_order':
        ...
        return audio_to_text_SKD.RandomizedChainDataset(datasets=datasets, rnd_seed=0)
    elif bucketing_strategy == 'fully_randomized':
        return audio_to_text_SKD.RandomizedChainDataset(datasets=datasets, rnd_seed=random.randint(0, 30000))
    else:
        raise ValueError(
