...
from nemo.collections.asr.data import audio_to_text_dali
...
def get_concat_char_dataset_SelfAttn(
    ...
    for manifest_filepath in manifest_filepaths:
        ...
        dataset = get_char_dataset_SelfAttn(config=conf, augmentor=augmentor)
...
def get_char_dataset_SelfAttn(config: dict, augmentor: Optional['AudioAugmentor'] = None) -> audio_to_text_SelfAttn.AudioToCharDataset_SelfAttn:
    """
    ...
    dataset = audio_to_text_SelfAttn.AudioToCharDataset_SelfAttn(
...
def get_concat_bpe_dataset_SelfAttn(
    config: dict,
    ...
    for manifest_filepath in manifest_filepaths:
        conf = copy.deepcopy(config)
        conf['manifest_filepath'] = manifest_filepath
        dataset = get_bpe_dataset_SelfAttn(config=conf, tokenizer=tokenizer, augmentor=augmentor)
        datasets.append(dataset)
...
def get_bpe_dataset_SelfAttn(
    config: dict, tokenizer: 'TokenizerSpec', augmentor: Optional['AudioAugmentor'] = None
) -> audio_to_text_SelfAttn.AudioToBPEDataset_SelfAttn:
    """
    ...
    """
    dataset = audio_to_text_SelfAttn.AudioToBPEDataset_SelfAttn(
        manifest_filepath=config['manifest_filepath'],
        tokenizer=tokenizer,
        sample_rate=config['sample_rate'],
        int_values=config.get('int_values', False),
...
def get_tarred_dataset(
    ...
    tokenizer: Optional['TokenizerSpec'] = None,
    augmentor: Optional['AudioAugmentor'] = None,
) -> Union[audio_to_text_SelfAttn.TarredAudioToBPEDataset, audio_to_text_SelfAttn.TarredAudioToCharDataset]:
    """
    ...
    for dataset_idx, (tarred_audio_filepath, manifest_filepath) in enumerate(
        ...
        if tokenizer is None:
            dataset = audio_to_text_SelfAttn.TarredAudioToCharDataset(
            ...
            dataset = audio_to_text_SelfAttn.TarredAudioToBPEDataset(
                audio_tar_filepaths=tarred_audio_filepath,
                manifest_filepath=manifest_filepath,
                tokenizer=tokenizer,
                sample_rate=config['sample_rate'],
...
def get_audio_to_text_char_dataset_from_config_SelfAttn(
    ...
    if config.get('is_tarred', False):
        ...
        if 'manifest_filepath' in config and config['manifest_filepath'] is None:
            ...
            return None
        if is_concat:
            dataset = get_concat_char_dataset_SelfAttn(
                config=config, global_rank=global_rank, world_size=world_size, augmentor=augmentor
            )
        else:
            dataset = get_char_dataset_SelfAttn(config=config, augmentor=augmentor)
    return dataset


def get_audio_to_text_bpe_dataset_from_config_SelfAttn(
    config,
    local_rank: int,
    global_rank: int,
    world_size: int,
    ...
    if config.get('is_tarred', False):
        ...
        if 'manifest_filepath' in config and config['manifest_filepath'] is None:
            ...
            return None
        if is_concat:
            dataset = get_concat_bpe_dataset_SelfAttn(
                config=config,
                global_rank=global_rank,
                world_size=world_size,
                tokenizer=tokenizer,
            ...
            dataset = get_bpe_dataset_SelfAttn(config=config, tokenizer=tokenizer, augmentor=augmentor)
    return dataset
...
def get_chain_dataset(datasets, ds_config):
    if len(datasets) > 1:
        if ds_config.get('bucketing_batch_size', None) is not None:
            ...
            for idx, dataset in enumerate(datasets):
                datasets[idx] = audio_to_text_SelfAttn.BucketingDataset(
    ...
    if bucketing_strategy == 'fixed_order':
        ...
        return audio_to_text_SelfAttn.RandomizedChainDataset(datasets=datasets, rnd_seed=0)
    elif bucketing_strategy == 'fully_randomized':
        return audio_to_text_SelfAttn.RandomizedChainDataset(datasets=datasets, rnd_seed=random.randint(0, 30000))
    else:
        raise ValueError(
