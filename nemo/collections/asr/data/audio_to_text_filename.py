...
__all__ = [
    'AudioToCharDataset_filename',
    'AudioToBPEDataset_filename',
    'TarredAudioToCharDataset',
    'TarredAudioToBPEDataset',
]


def _speech_collate_fn_filename(batch, pad_id):
    ...
    if len(packed_batch) == 5: 
        _, audio_lengths, _, tokens_lengths, filename = packed_batch
    ...
    #jykang
    return audio_signal, audio_lengths, tokens, tokens_lengths, filename
...
class _AudioTextDataset_filename(Dataset):
    """
    ...
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        ...
        return {
            ...
            # filename
            "filename": NeuralType(tuple('B'), StringType())
        }
    ...
    def __getitem__(self, index):
        ...
        if self.return_sample_id:
            ...
            # filename 
            output = f, fl, torch.tensor(t).long(), torch.tensor(tl).long(), filename
    ...
    def _collate_fn(self, batch):
        return _speech_collate_fn_filename(batch, pad_id=self.manifest_processor.pad_id)


class AudioToCharDataset_filename(_AudioTextDataset_filename):
    """
...
class AudioToBPEDataset_filename(_AudioTextDataset_filename):
    """
    Dataset that loads tensors via a json file containing paths to audio
    files, transcripts, and durations (in seconds). Each new line is a
    different sample. Example below:
