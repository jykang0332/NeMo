...
__all__ = [
    'AudioToCharDataset_KD',
    'AudioToBPEDataset_KD',
    'TarredAudioToCharDataset',
    'TarredAudioToBPEDataset',
]


def _speech_collate_fn_KD(batch, pad_id):
    ...
    if len(packed_batch) == 5: 
        ...
        #jykang
        _, audio_lengths, _, tokens_lengths, teacher_softmax = packed_batch

        # test ver
        # _, audio_lengths, _, tokens_lengths, teacher_softmax, file_name = packed_batch
    ...
    #jykang
    # compute max_softmax_len
    max_softmax_len = 0
    has_softmax = teacher_softmax[0] is not None
    if has_softmax:
        for i in range(len(teacher_softmax)):
            temp_softmax_len = teacher_softmax[i].shape[0]
            # print(temp_softmax_len)
            if temp_softmax_len > max_softmax_len:
                max_softmax_len = temp_softmax_len
        # print("target length: ", max_softmax_len)
    ...
    for b in batch:
        if len(b) == 5:
            ...
            #jykang
            sig, sig_len, tokens_i, tokens_i_len, softmax_i = b

            # test ver
            # sig, sig_len, tokens_i, tokens_i_len, softmax_i, _ = b
        else:
        ...
        #jykang
        # teacher_softmax padding
        softmax_i_len = softmax_i.shape[0]
        if softmax_i_len < max_softmax_len:
            pad = (0, 0, 0, max_softmax_len - softmax_i_len)
            softmax_i = torch.nn.functional.pad(softmax_i, pad, 'constant', 0)
        softmax_padded.append(softmax_i)
    ...
    #jykang
    if has_softmax:
        softmax_padded = torch.stack(softmax_padded)
    ...
    #jykang
    return audio_signal, audio_lengths, tokens, tokens_lengths, softmax_padded
...
class _AudioTextDataset_KD(Dataset):
    """
    ...
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        ...
        return {
            ...
            #jykang
            'teacher_softmax': NeuralType(('B', 'T', 'C'), ProbsType()),

            # test ver
            #"file_name": NeuralType(tuple('B'), StringType())
        }
    ...
    def __getitem__(self, index):
        ...
        if self.return_sample_id:
            ...
            #jykang
            output = f, fl, torch.tensor(t).long(), torch.tensor(tl).long(), teacher_softmax

            # test ver
            # output = f, fl, torch.tensor(t).long(), torch.tensor(tl).long(), teacher_softmax, file_name
    ...
    def _collate_fn(self, batch):
        return _speech_collate_fn_KD(batch, pad_id=self.manifest_processor.pad_id)


class AudioToCharDataset_KD(_AudioTextDataset_KD):
    """
...
class AudioToBPEDataset_KD(_AudioTextDataset_KD):
    """
    Dataset that loads tensors via a json file containing paths to audio
    files, transcripts, and durations (in seconds). Each new line is a
    different sample. Example below:
