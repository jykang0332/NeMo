...
def _speech_collate_fn(batch, pad_id):
    ...
    sample_ids = None
    _, audio_lengths, _, tokens_lengths, _, softmaxs_lengths = packed_batch
    ...
    max_softmax_len = max(softmaxs_lengths).item()

    # audio_signal, tokens = [], []
    audio_signal, tokens, softmaxs = [], [], []
    for b in batch:
        # if len(b) == 5:
        ...
        sig, sig_len, tokens_i, tokens_i_len, softmaxs_i, softmaxs_i_len = b
        ...
        softmaxs_i_len = softmaxs_i_len.item()
        if softmaxs_i_len < max_softmax_len:
            pad = (0, 0, 0, max_softmax_len - softmaxs_i_len)
            softmaxs_i = torch.nn.functional.pad(softmaxs_i, pad)
        softmaxs.append(softmaxs_i)
    ...
    softmaxs = torch.stack(softmaxs)
    softmaxs_lengths = torch.stack(softmaxs_lengths)
    ...
    return audio_signal, audio_lengths, tokens, tokens_lengths, softmaxs, softmaxs_lengths
...
class _AudioTextDataset_KD(Dataset):
    ...
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        ...
        return {
            ...
            'transcript_length': NeuralType(tuple('B'), LengthsType()),
            # jykang
            'te_softmax': NeuralType(('B', 'T', 'C'), ProbsType()), # teacher softmax
            'te_softmax_len': NeuralType(tuple('B'), LengthsType()), # teacher softmax length
            'sample_id': NeuralType(tuple('B'), LengthsType(), optional=True),
        }
    ...
    def __getitem__(self, index):
        ...
        # Load te_softmax
        file_name = sample.audio_file.split('/')[-2:]
        file_name[0] = file_name[0].replace('-processed', '')
        file_name[1] = file_name[1].replace('.wav', '.npy')
        file_name = '/'.join(file_name)

        te_softmax = np.load('/home/jykang/NeMo/data/softmax/' + file_name)
        te_softmax = torch.tensor(te_softmax)
        te_softmax_len = torch.tensor(te_softmax.shape[0]).long()
        ...
        output = f, fl, torch.tensor(t).long(), torch.tensor(tl).long(), te_softmax, te_softmax_len
