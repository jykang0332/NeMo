...
#jykang
from nemo.collections.asr.data import audio_to_text_filename
...
def get_concat_char_dataset(
    ...
    for manifest_filepath in manifest_filepaths:
        ...
        dataset = get_char_dataset_filename(config=conf, augmentor=augmentor)
...
def get_char_dataset_filename(config: dict, augmentor: Optional['AudioAugmentor'] = None) -> audio_to_text_filename.AudioToCharDataset_filename:
    """
    ...
    dataset = audio_to_text_filename.AudioToCharDataset_filename(
...
def get_concat_bpe_dataset_filename(
    config: dict,
    ...
    for manifest_filepath in manifest_filepaths:
        conf = copy.deepcopy(config)
        conf['manifest_filepath'] = manifest_filepath
        dataset = get_bpe_dataset_filename(config=conf, tokenizer=tokenizer, augmentor=augmentor)
        datasets.append(dataset)
...
def get_bpe_dataset_filename(
    config: dict, tokenizer: 'TokenizerSpec', augmentor: Optional['AudioAugmentor'] = None
) -> audio_to_text_filename.AudioToBPEDataset_filename:
    """
    ...
    """
    dataset = audio_to_text_filename.AudioToBPEDataset_filename(
        manifest_filepath=config['manifest_filepath'],
        tokenizer=tokenizer,
        sample_rate=config['sample_rate'],
        int_values=config.get('int_values', False),
...
def get_audio_to_text_char_dataset_from_config_filename(
    ...
    if config.get('is_tarred', False):
        ...
        if is_concat:
            ...
            dataset = get_char_dataset_filename(config=config, augmentor=augmentor)
    return dataset


def get_audio_to_text_bpe_dataset_from_config_filename(
    config,
    local_rank: int,
    global_rank: int,
    world_size: int,
    ...
    if config.get('is_tarred', False):
        ...
        if 'manifest_filepath' in config and config['manifest_filepath'] is None:
            ...
            return None
        if is_concat:
            dataset = get_concat_bpe_dataset_filename(
                config=config,
                global_rank=global_rank,
                world_size=world_size,
                tokenizer=tokenizer,
            ...
            dataset = get_bpe_dataset_filename(config=config, tokenizer=tokenizer, augmentor=augmentor)
    return dataset
