...
def get_concat_char_dataset(
    ...
    for manifest_filepath in manifest_filepaths:
        ...
        dataset = get_char_dataset_KD(config=conf, augmentor=augmentor)
...
def get_char_dataset_KD(config: dict, augmentor: Optional['AudioAugmentor'] = None) -> audio_to_text_KD.AudioToCharDataset_KD:
    """
    ...
    dataset = audio_to_text_KD.AudioToCharDataset_KD(
...
def get_bpe_dataset_KD(
    ...
    """
    dataset = audio_to_text_KD.AudioToBPEDataset_KD(
        manifest_filepath=config['manifest_filepath'],
        tokenizer=tokenizer,
        sample_rate=config['sample_rate'],
        int_values=config.get('int_values', False),
...
def get_audio_to_text_char_dataset_from_config_KD(
    ...
    if config.get('is_tarred', False):
        ...
        if is_concat:
            ...
            dataset = get_char_dataset_KD(config=config, augmentor=augmentor)
    return dataset


def get_audio_to_text_bpe_dataset_from_config_KD(
    config,
    local_rank: int,
    global_rank: int,
    world_size: int,
    ...
    if config.get('is_tarred', False):
        ...
        if is_concat:
            ...
            dataset = get_bpe_dataset_KD(config=config, tokenizer=tokenizer, augmentor=augmentor)
    return dataset
