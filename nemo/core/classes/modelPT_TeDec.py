...
class ModelPT(LightningModule, Model):
    ...
    @rank_zero_only
    def maybe_init_from_pretrained_checkpoint_TeDec(self, cfg: OmegaConf, map_location: str = 'cpu'):
        """
        Initializes a given model with the parameters obtained via specific config arguments.
        The state dict of the provided model will be updated with `strict=False` setting so as to prevent
        requirement of exact model parameters matching.
        ...
        if 'init_from_nemo_model' in cfg and cfg.init_from_nemo_model is not None:
            with open_dict(cfg):
                if isinstance(cfg.init_from_nemo_model, str):
                    ...
                    te_dec_bias = torch.load('/home/jykang/NeMo/data/decoder/te_dec_bias.pt')
                    te_dec_weight = torch.load('/home/jykang/NeMo/data/decoder/te_dec_weight.pt')

                    with torch.no_grad():
                        self.state_dict()['decoder.decoder_layers.0.bias'].copy_(te_dec_bias)
                        self.state_dict()['decoder.decoder_layers.0.weight'].copy_(te_dec_weight)
